...
test('published ADAMS citation supports identity only and preserves original dates', async () => {
  const release = JSON.parse(await readFile(new URL('../data/atlas-release.json', import.meta.url), 'utf8'))
  const record = release.stages.operations.records.find(r => r.id === 'ops_nrc_05000220_2025q1')
  const citation = record.citations.find(c => c.id === 'cite_adams_ml26173a204_identity')
  assert.ok(citation)
  assert.deepEqual(citation.supportsFields, ['reactor_name'])
  assert.equal(citation.sourceDateOriginal, '2026-06-22')
  assert.equal(citation.effectiveDate, null)
  assert.match(citation.locator, /05000220/)
  assert.equal(citation.url, 'https://www.nrc.gov/docs/ML2617/ML26173A204.pdf')
})
